...
void ACameraDirector::Tick(float DeltaTime)
{
	...
	const float TimeBetweenCamera = 5.0f;
	const float SmoothBlend = 0.75f;

	TimeToNextCamera -= DeltaTime;
	if (TimeToNextCamera <= 0.0f) {
		FChangeCameraData NowCamera = Cameras[NowCameraIndex];

		TimeToNextCamera += NowCamera.TimeBetweenCameraChanges;
		...
		//		controller->SetViewTargetWithBlend(CameraTwo, SmoothBlend);
		//	}
		}
		NowCameraIndex++;
